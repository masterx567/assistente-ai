...
**Check-in automatico (anti-bugia)**: `POST /api/gym-webhook?secret=GYM_WEBHOOK_SECRET`, body `{"tipo":<tipo grezzo>,"minuti":N,"data":"YYYY-MM-DD"}`. Pensato per uno Shortcut Apple triggerato da "inizio o termino allenamento" in Salute (HealthKit) + azione "Trova campione di dati sanitari" per estrarre Tipo di allenamento/Durata/Data di inizio (occhio: nel property picker Shortcuts c'è anche "Conteggio", che vale sempre 1 — non va confuso con "Durata"). Validazione server: `data` deve essere oggi, richiede `minuti>=30`. Parsing `minuti` tollerante (regex fallback se arriva come stringa). Rifiuti loggati su BotErrors col body grezzo per debug. Il trigger scatta 2x per allenamento (inizio+fine): innocuo, `checkin()` è idempotente 1x/giorno.
