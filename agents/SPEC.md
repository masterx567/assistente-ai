...
- `enable_banking.py::sync_transactions(days_back, account)` — parametrizzata per conto, sceglie `EB_ACCOUNT_UID`/`EB_SESSION_ID` (Isybank) o `EB_REVOLUT_ACCOUNT_UID`/`EB_REVOLUT_SESSION_ID` (Revolut). Solo Isybank ricalcola il saldo dall'ancora fissa (`ISYBANK_ANCHOR_DATE`/`ISYBANK_ANCHOR_BALANCE`) — Revolut non ha saldo tracciato, serve solo per le transazioni.
...
- `get_monthly_spending`/`_get_spending`, `get_monthly_cashflow`/`_sum_cashflow_range` e `_get_transactions_since` escludono client-side la categoria `"Trasferimento"` — funzionano automaticamente su dati combinati dei due conti, nessuna modifica extra necessaria (sommano per data/categoria, non per conto).
