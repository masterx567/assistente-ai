...
**Check-in automatico (anti-bugia)**: `POST /api/gym-webhook?secret=GYM_WEBHOOK_SECRET`, nessun body richiesto. Pensato per uno Shortcut Apple triggerato da "inizio o termino allenamento" in Salute (HealthKit): il trigger stesso è la prova che l'allenamento è reale, niente da validare dal telefono (tipo/durata/data estratti da Shortcuts si sono rivelati troppo fragili — property picker inconsistente su iOS, valori sbagliati). Sempre loggato come `checkin("palestra")`, data = server al momento della chiamata. Il trigger scatta 2x per allenamento (inizio+fine): innocuo, `checkin()` è idempotente 1x/giorno.
